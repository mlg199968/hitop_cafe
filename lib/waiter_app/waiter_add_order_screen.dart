  ...
  void saveBillOnLocalStorage({String? id}) async {
    ...
    if (UserTools.userPermission(context,count: HiveBoxes.getOrders().values.length)) {
      if (items.isNotEmpty) {
        ...
        Pack pack=Pack()..object=[orderBill.toJson()]..type="order"..message=orderBill.orderDate.toString()..packId=orderBill.orderId;
        HiveBoxes.getPack().put(pack.packId, pack);
        bool res=Provider.of<ClientProvider>(context,listen: false).sendPack(pack);
        if(!res){
          showSnackBar(context, "ارتباط با سرور برقرار نیست!",type: SnackType.error);
        }
