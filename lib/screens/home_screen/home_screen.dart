...
class _HomeScreenState extends State<HomeScreen> with WindowListener {
  ...
  onTapFunction(int index, VoidCallback onTap, {bool permission = true}) async {
    DateTime time = await BackendServices.getServerTime();
    print(time);
    if (permission) {
      screenIndex = index;
                ...
                onYes: () async {
                  if (userProvider.saveBackupOnExist) {
                    await BackupTools(quickBackup: true).createBackup(
                      context,
                      directory: userProvider.backupDirectory,
                    );
                  }
          ...
          title: Row(
            ...
            children: [
              ...
              Flexible(
                  child: Text(
                "HITOP CAFE ",
                style: GoogleFonts.aBeeZee(fontSize: 23, color: Colors.black),
              )),
            ],
          ),
          actions: [
            ...
            Padding(
              ...
              child: ActionButton(
                ...
                onPress: () {
                  showDialog(
                      context: context,
                      builder: (context) => const ActiveUserPanel());
                },
              ),
            )
          ],
                                    ...
                                    onTap: () {
                                      onTapFunction(7, () {
                                        Navigator.pushNamed(
                                          ...
                                          permission: UserTools.userPermission(
                                              context,
                                              userTypes: kupBillScreen));
                                    }),
                                SmallCardButton(
                                    label: "فاکتور خرید",
                                    image: "bill.png",
